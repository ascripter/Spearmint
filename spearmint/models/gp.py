...
class GP(AbstractModel):
    ...
    def pi(self, pred, C=0, compute_grad=False):
        ...
        # norm.sf = 1 - norm.cdf
        prob = sps.norm.sf(old_div(C_minus_m,sigma))

        if not compute_grad:
            return prob
        else:
            # Gradient of pi w.r.t. GP mean
            g_p_m = old_div(sps.norm.pdf( old_div(C_minus_m, sigma) ), sigma)
            # Gradient of pi w.r.t. GP variance (equals grad w.r.t. sigma / (2*sigma))
            g_p_v = sps.norm.pdf( old_div(C_minus_m, sigma) ) * C_minus_m / sigma2 / (2*sigma)
